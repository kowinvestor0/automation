...
def render_crime_story_video(
    ...
    try:
        ...
        if ai_script and ai_script.get("scenes"):
            ...
            log(f"[StoryEngine] Ap dung kich ban doc ban tu Gemini API cho kenh '{channel_tag}' ({len(scenes_data)} scenes)")
        else:
            log(f"[StoryEngine] Su dung kich ban goc doc quyen (>60s) cua de tai '{case_name}' - dam bao 100% doc ban vi de tai nay chua tung dung cho kenh nao.")
    except Exception as e:
        log(f"[StoryEngine] Gemini gap loi ({e}), tiep tuc voi kich ban goc doc quyen cua de tai '{case_name}'.")
