...
MOODS = {
    # A minor: the default uneasy bed.
    "mysteries": {"bass": 55.00, "notes": [110.00, 164.81, 220.00], "lp": 1100, "pulse": 0.50},
    # G minor, darker and slower. Heavy low end, almost no movement.
    "truecrime": {"bass": 49.00, "notes": [98.00, 146.83, 196.00], "lp": 900, "pulse": 0.35},
    # C major: open and bright, keeps the pace up.
    "facts": {"bass": 65.41, "notes": [130.81, 196.00, 261.63], "lp": 1600, "pulse": 0.75},
    # B minor: somber but not threatening.
    "history": {"bass": 61.74, "notes": [123.47, 185.00, 246.94], "lp": 1300, "pulse": 0.60},
    # D major: steady and forward-leaning, the "explainer" bed.
...
def make_music(duration, mood, workdir):
    ...
    m = MOODS.get(mood, MOODS["mysteries"])
    dur = duration + 2.0
...
def build_sfx_track(timeline, total_duration, cfg, workdir):
    ...
    run(["ffmpeg", "-y", "-loglevel", "error", *inputs,
        ...
        cwd=workdir)
    log(f"{len(events)} sound effects ({ffprobe_duration(out):.1f}s)")
    return out
