...
def build_ass(timeline, cfg, out_path):
    # Same source of truth as the renderer, or the .ass would be laid out for a
    # different canvas than the video and every caption would sit off-centre.
    from .render import _dims

    w, h, _fps = _dims(cfg)
    # Font size is authored against 1080-wide, so it scales with the canvas.
