    ...
    for attempt in range(retries + 1):
        ...
        with open(dest, "wb") as f:
            for block in r.iter_content(1 << 16):
                f.write(block)
        return dest
    raise RuntimeError("429 tras reintentar")
    ...
    for query in queries:
        ...
        if query not in state["wiki_cache"]:
            try:
                ...
                log(f"Wikimedia fallo en '{query}': {str(e)[:80]}")
        ...
        for hit in state["wiki_cache"][query]:
            ...
            try:
                ...
                if "429" in str(e):
                    _wiki_penalize()
                log(f"no pude bajar {hit['title'][:38]}: {str(e)[:60]}")
                continue
            return {
...
def _pexels_get(path, params):
    ...
    if r.status_code == 401:
        _pexels_dead[0] = True
        raise RuntimeError("PEXELS_API_KEY invalida; sigo sin Pexels")
    r.raise_for_status()
    return r.json()
...
def _try_pexels_video(state, queries):
    ...
    for query in queries:
        try:
            ...
            log(f"Pexels video fallo en '{query}': {e}")
            continue
...
def _try_pexels_photo(state, queries):
    ...
    for query in queries:
        try:
            ...
            log(f"Pexels foto fallo en '{query}': {e}")
            continue
...
def fetch_for_timeline(timeline, cfg, subject=None):
    ...
    # El humor pide gente y situaciones cotidianas -> Pexels manda ahi.
    # Los misterios piden el lugar de verdad -> Wikimedia manda.
    ...
    if "wikimedia" in priority:
        log(f"Wikimedia Commons (sin llave), tema: {subject or 'n/d'}")
    if not _pexels_key() and "pexels_video" in priority:
        log("sin PEXELS_API_KEY -> me salto el video de stock")
    ...
    for sc in timeline:
        ...
        assets.append(asset)
        log(f"escena {sc['index'] + 1}: {asset['source']}/{asset['kind']} <- {asset['query']}")
